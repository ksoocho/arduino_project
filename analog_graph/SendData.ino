    ...
    void SendData(int Command, unsigned int yValue,unsigned int xValue){
     ...
     /* >=================================================================< 
       ...
       if( Checksum !=0 ){
        Serial.print(0);            // send start bit 
        Serial.print(Command);      // command eg: Which Graph is this data for
        
        Serial.print(yMSB);         // Y value's most significant byte  
        Serial.print(yLSB);         // Y value's least significant byte   
        Serial.print(xMSB);         // X value's most significant byte  
        Serial.print(xLSB);         // X value's least significant byte  
        
        Serial.print(zeroByte);     // Which values have a zero value
        Serial.print(Checksum);     // Error Checking Byte
       }
    }  
 ...
 void PlottArray(unsigned int Cmd,unsigned int Array1[],unsigned int Array2[]){
   
      SendData(Cmd+1, 1,1);                        // Tell PC an array is about to be sent                      
      delay(1);
      for(int x=0;  x < sizeOfArray;  x++){     // Send the arrays 
        SendData(Cmd, Array1[x],Array2[x]);
        delay(1);
      }
